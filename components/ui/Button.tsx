...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const variantStyles = {
      ...
      secondary:
        "bg-[var(--bg-subtle)] text-[var(--text-primary)] hover:bg-[var(--border-default)]",
      outline:
        "border border-[var(--border-default)] text-[var(--text-primary)] hover:bg-[var(--bg-subtle)] hover:border-[var(--border-strong)]",
      ghost:
        "text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-subtle)]",
      destructive:
        "bg-[var(--status-error-bg)] text-[var(--status-error)] hover:opacity-90 border border-red-200 dark:border-red-900/40",
    };

    const sizeStyles = {
      sm: "text-xs px-3 py-1.5 min-h-[34px] gap-1.5",
      md: "text-xs sm:text-sm px-4 py-2 min-h-[40px] gap-2",
      lg: "text-sm sm:text-base px-5 py-2.5 min-h-[46px] gap-2.5",
    };
