...
- [x] Write `TrothEscrow.sol` core logic:
  ...
  - Refund & Cancellation: `cancelUnclaimed`, `contractorRefund`, and `claimDeadlineRefund`.
  - Input validation ensuring all milestone deadlines are in the future.
- [x] Write unit tests for all contract flows (100% test pass: 11/11 unit tests passing).
...
- [x] Build **Create Escrow Page (`/create`)**:
  - Title, milestone builder (dynamic add/remove milestones with realistic $5–$10 amounts and delivery durations), recipient toggle (Direct Address vs Claim Link).
  - Mutual protection safeguards breakdown (contractor review window vs client deadline countdown).
  - USDC token approval + contract call transaction orchestration.
- [x] Build **Agreement Detail & Dashboard (`/agreement/[id]`)**:
  - Payer View: Review submitted proofs, Approve & Release button, Reclaim Expired Milestone action button.
...
- [x] Verify 100% test coverage with Foundry (11/11 unit tests passing).
...
- [x] On-chain contract broadcast to Arc Mainnet (`0xF7A20db6E51171263856f282ed188f795206d649`).
...
- [x] Document live Netlify endpoints, Arc RPCs, contract address, and GitHub repository in `README.md`.

---

## Done Definition
The project is **DONE** when:
1. `TrothEscrow.sol` has 100% test pass rate (11/11 tests) and is broadcasted to Arc Mainnet.
